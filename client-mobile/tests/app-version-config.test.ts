...
test("原生平台版本与 Expo 配置保持一致", async () => {
  ...
  assert.match(androidGradle, /versionName "1\.5\.0"/)
  assert.equal(iosProject.match(/CURRENT_PROJECT_VERSION = 13;/g)?.length, 2)
  assert.equal(iosProject.match(/MARKETING_VERSION = 1\.5\.0;/g)?.length, 2)
  assert.match(iosInfo, /<key>CFBundleShortVersionString<\/key>\s*<string>1\.5\.0<\/string>/)
  assert.match(iosInfo, /<key>CFBundleVersion<\/key>\s*<string>13<\/string>/)
})
