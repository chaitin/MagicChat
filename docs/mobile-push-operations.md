...
密钥为空时推送关闭，配置有效密钥后推送启用。Server 首次启用时会自动生成内部凭据加密密钥，写入宿主机 `./data/server/push/credential.key`；该内部密钥不发送给 Gateway，也不需要部署者配置。备份和恢复私有 Server 时必须保留整个 `data` 目录，否则数据库中的既有 Grant 无法解密，移动客户端需要重新注册。
