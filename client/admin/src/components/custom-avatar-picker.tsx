...
import { MinusIcon, PlusIcon, RotateCcwIcon, UploadIcon } from "lucide-react"
...
type ImageLayout = { displayHeight: number; displayWidth: number }
...
export function CustomAvatarPicker({
  ...
  disabled?: boolean
  onChange: (avatar: CroppedAvatar | null) => void
  ...
  const dragStartRef = React.useRef<{ offset: Point; pointer: Point } | null>(
    null
  )
  ...
  React.useLayoutEffect(() => {
    ...
    function updateCropFrameSize() {
      const nextSize = cropFrame?.getBoundingClientRect().width ?? 0
      if (nextSize > 0) setCropFrameSize(nextSize)
    }
  ...
  React.useEffect(() => {
    ...
    if (previousCropSize === cropFrameSize) return
    ...
    setOffset((currentOffset) =>
      clampOffset(
        { x: currentOffset.x * ratio, y: currentOffset.y * ratio },
        nextLayout,
        cropFrameSize
      )
    )
  ...
  function handlePointerDown(event: React.PointerEvent<HTMLDivElement>) {
    if (interactionsDisabled || !imageLayout) return
  ...
  function handlePointerMove(event: React.PointerEvent<HTMLDivElement>) {
    if (!dragStartRef.current || !imageLayout) return
  ...
  async function handleApply() {
    if (!sourceFile || !imageLayout || !imageRef.current) return
      ...
      <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_auto]">
        <div
          ...
          className={cn(
            ...
            sourceUrl ? "touch-none" : "transition-colors hover:bg-muted/80",
            sourceUrl &&
        ...
        >
          {sourceUrl ? (
            ...
            <button
              className="absolute inset-0 flex flex-col items-center justify-center gap-3 px-4 text-sm text-muted-foreground disabled:opacity-50"
...
async function createCroppedAvatar({
  ...
  const blob = await new Promise<Blob | null>((resolve) =>
    canvas.toBlob(resolve, avatarOutputType, avatarOutputQuality)
  )
  if (!blob) throw new Error("avatar conversion failed")

  return {
    file: new File([blob], createAvatarFileName(sourceFile.name), {
      type: avatarOutputType,
    }),
    previewUrl: canvas.toDataURL(avatarOutputType, avatarOutputQuality),
  }
}
