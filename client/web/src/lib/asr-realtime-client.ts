export type ASRRealtimeState =
  "connecting" | "recognizing" | "committed" | "completed" | "failed"
...
const backpressureThresholdBytes = 256 * 1024
const maxQueuedAudioBytes = 2 * 1024 * 1024
...
export class ASRRealtimeClient {
  ...
  constructor(options: ASRRealtimeClientOptions = {}) {
    ...
    this.url = options.url ?? buildASRWebSocketURL(window.location)
  }
  ...
  sendAudio(audio: ArrayBuffer) {
    ...
    if (
      ...
      this.socket.bufferedAmount > backpressureThresholdBytes
    ) {
  ...
  private enqueueAudio(audio: ArrayBuffer) {
    if (this.queuedAudioBytes + audio.byteLength > maxQueuedAudioBytes) {
...
function buildASRWebSocketURL(location: Location) {
  const protocol = location.protocol === "https:" ? "wss:" : "ws:"
  return `${protocol}//${location.host}/api/client/asr/realtime`
}
