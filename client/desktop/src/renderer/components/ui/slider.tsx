    ...
    >
      ...
      {Array.from({ length: _values.length }, (_, index) => (
        <SliderPrimitive.Thumb
          ...
          key={index}
          className="block size-3 shrink-0 rounded-full border border-primary bg-white shadow-sm ring-ring/50 transition-[color,box-shadow] select-none hover:ring-2 focus-visible:ring-2 focus-visible:outline-hidden disabled:pointer-events-none disabled:opacity-50"
        />
      ))}
    </SliderPrimitive.Root>
  )
