                ...
                {conversations.map((conversation) => {
                  ...
                  return (
                    <li key={conversation.id}>
                      ...
                      >
                        <span className="flex size-10 shrink-0 items-center justify-center rounded-full bg-card text-sm font-semibold shadow-sm">
                          {Array.from(conversation.name)[0]}
                        </span>
  ...
  return (
    <aside className="flex w-12 shrink-0 flex-col items-center border-r bg-sidebar py-3">
      <BeButton
        ...
        variant="ghost"
        size="icon"
        className="mb-6 rounded-lg bg-card text-xs font-semibold disabled:opacity-100"
        disabled
        aria-label="用户菜单"
        title={userName}
      >
        {Array.from(userName)[0] ?? "我"}
      </BeButton>
